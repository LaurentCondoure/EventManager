...
/// <summary>
/// Tests for UpdateEventInputValidator, covering all BR1 rules applied to event updates.
/// </summary>
public class UpdateEventInputValidatorTests
{
    private readonly UpdateEventInputValidator _sut = new();

    private static UpdateEventInput Valid() => new(
        Title:       "Concert Jazz",
    ...
    [Fact]
    public void Location_Empty_ShouldFail()
        => _sut.TestValidate(Valid() with { Location = "" })
               .ShouldHaveValidationErrorFor(x => x.Location);

    [Fact]
    public void Location_ExceedsMaxLength_ShouldFail()
        => _sut.TestValidate(Valid() with { Location = new string('a', 201) })
               .ShouldHaveValidationErrorFor(x => x.Location);

    [Fact]
    public void Location_Valid_ShouldPass()
        => _sut.TestValidate(Valid())
    ...
    [Fact]
    public void ArtistName_Null_ShouldPass()
        => _sut.TestValidate(Valid() with { ArtistName = null })
               .ShouldNotHaveValidationErrorFor(x => x.ArtistName);

    [Fact]
    public void ArtistName_ExceedsMaxLength_ShouldFail()
        => _sut.TestValidate(Valid() with { ArtistName = new string('a', 201) })
               .ShouldHaveValidationErrorFor(x => x.ArtistName);

    [Fact]
    public void ArtistName_Valid_ShouldPass()
        => _sut.TestValidate(Valid())
               .ShouldNotHaveValidationErrorFor(x => x.ArtistName);
}
