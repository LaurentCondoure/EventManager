...
sequenceDiagram
    ...
    alt Varnish cache HIT (TTL 5min)
        ...
        Varnish->>EH: GET /api/events?page=1
        EH->>API: GET /api/events?page=1
        ...
        Cache->>Redis: GET events:list:version
        Redis-->>Cache: n (current version, 0 if not set)
        Cache->>Redis: GET events:page:1:size:20:v{n}
        alt Redis cache hit
            Redis-->>Cache: JSON
            ...
            alt DB error
                ...
                Cache->>Redis: SET events:page:1:size:20:v{n} TTL 10min
                Cache-->>Service: IEnumerable<Event>
            end
        end
