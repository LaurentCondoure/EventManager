...
The proxy forwards every request starting with `/api` from the Vite dev server (`localhost:5173`) to the API (`localhost:5000`). This means:
...
In production (IIS), the frontend is served from `localhost:8080` and the API from `localhost:5000` — two distinct origins. The browser enforces the *Same-Origin Policy*: it blocks responses from a different origin unless the server explicitly permits it via CORS headers.
...
The store manages the event list state for `HomeView` — particularly infinite scroll pagination.
...
`hasMore` is derived from whether the API returned a full page. If `pageSize = 20` and only 12 events came back, there are no more pages.

`EventDetailView` does not use the store — it fetches directly via `eventService.getFull()` since detail data is not shared with other views.
...
After posting, `getFull` is called again to reload the full event with the newly created comment. This is deliberate: the POST response does not include the full comment list, and the reloaded data ensures `id` and `createdAt` (set server-side) are present. A local push into `data.value.comments` would be more efficient but would require the POST to return the created comment with its server-assigned fields. This is a known trade-off, to be revisited after Day 3 (Varnish + Docker in place).
...
In edit mode, the existing event is fetched and pre-fills the form. Two field transformations are applied in reverse compared to submission:
